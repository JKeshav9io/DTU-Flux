...
class HomeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final studentId = student['id'] ?? ''; // Defensive fallback
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        final attendance = snapshot.data?.where((a) =>
        a['date'] == today && a['held'] == 'conducted' && a['status'] == 'present'
        ).toList() ?? [];
        if (attendance.isEmpty) {
          return _infoCard(context, title: "Today's Attendance", value: "No class", progress: 0, valueColor: Colors.grey);
        }
        final percent = attendance.length / (attendance.length == 0 ? 1 : attendance.length); // always 100% for attended
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              // Remove subject display, only show count and progress
              Row(
                children: [
                  Expanded(
                  ...
                  Text('${(percent * 100).toStringAsFixed(0)}%', style: theme.textTheme.bodyMedium?.copyWith(color: Colors.green, fontWeight: FontWeight.bold)),
                ],
              ),
            ],
          ),
...
// Campus Events Card
class CampusEventsCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<List<Map<String, dynamic>>>(
      stream: dataService.fetchEventsStream(),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
        ...
        if (snapshot.hasError) {
          ...
          return _eventCard(context, imageUrl: "", title: "Error", date: "");
        }
        final events = snapshot.data?.where((e) => e['isMVP'] == true).toList() ?? [];
        if (events.isEmpty) {
          return _eventCard(context, imageUrl: "", title: "No MVP events", date: "");
        }
        return Column(
          children: events.map((event) {
            final imageUrl = event['imageURL'] ?? '';
            final title = event['eventName'] ?? '';
            final dateTime = event['dateTime'];
            final location = event['location'] ?? '';
            String formattedDate = '';
            if (dateTime != null) {
              if (dateTime is Timestamp) {
                formattedDate = DateFormat.yMMMMd().add_jm().format(dateTime.toDate());
            ...
            return GestureDetector(
              onTap: () async {
                ...
                if (url != null && await canLaunchUrl(Uri.parse(url))) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('Could not launch event link')),
                  );
                }
              },
              child: _eventCard(
